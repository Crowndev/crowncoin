...
// Recursively determine the rounds of a given input (How deep is the Legacysend chain for a given input)
int CWallet::GetRealInputLegacysendRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("legacysend", "GetInputLegacysendRounds INSERTING %s\n", hash.ToString());
        ...
        if(nout >= wtx->vout.size())
        {
            ...
            LogPrint("legacysend", "GetInputLegacysendRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputLegacysendRounds(in2, rounds+1);
...
int CWallet::GetInputLegacysendRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realLegacysendRounds = GetRealInputLegacysendRounds(in, 0);
    return realLegacysendRounds > nLegacysendRounds ? nLegacysendRounds : realLegacysendRounds;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputLegacysendRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputLegacysendRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nLegacysendRounds;
            }
        }
    }
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(int64_t d, legacySendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(coin_type == ONLY_DENOMINATED) {
                ...
                if(rounds < nLegacysendRounds) continue;
            }
    ...
    if(coin_type == ONLY_DENOMINATED) {
        ...
        BOOST_FOREACH(int64_t v, legacySendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nLegacysendRoundsMin, int nLegacysendRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputLegacysendRounds(vin);
            if(rounds >= nLegacysendRoundsMax) continue;
            if(rounds < nLegacysendRoundsMin) continue;
            ...
            if(fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1){ //if fulfilled
                ...
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100*COIN)+10000)) {fAccepted = true;}
...
bool CWallet::SelectCoinsLegacy(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nLegacysendRoundsMin, int nLegacysendRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nLegacysendRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT10000IFMN : ONLY_DENOMINATED);
...
{
    return  nInputAmount != 0 && nInputAmount % LEGACYSEND_COLLATERAL == 0 && nInputAmount < LEGACYSEND_COLLATERAL * 5 && nInputAmount > LEGACYSEND_COLLATERAL;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: Legacysend requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - LEGACYSEND_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - LEGACYSEND_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH(CTxIn v, txCollateral.vin) {
        if(!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CLegacysendPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough Legacysend non-denominated funds for this transaction that are not equal 10000 CRW.");
                    } else {
                        strFailReason = _("Unable to locate enough Legacysend denominated funds for this transaction.");
                        strFailReason += " " + _("Legacysend uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }
...
string CWallet::PrepareLegacysendDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if(legacySendPool.GetState() != POOL_STATUS_ERROR && legacySendPool.GetState() != POOL_STATUS_SUCCESS)
        if(legacySendPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the Legacysend pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(legacySendPool.sessionDenom, 0.1*COIN, LEGACYSEND_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PrepareLegacysendDenominate - preparing legacysend denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        BOOST_FOREACH(int64_t v, legacySendDenominations){
            ...
            bool fAccepted = false;
            if((legacySendPool.sessionDenom & (1 << 0))      && v == ((1000*COIN) +1000000)) {fAccepted = true;}
            else if((legacySendPool.sessionDenom & (1 << 1)) && v == ((100*COIN)  +10000)) {fAccepted = true;}
            else if((legacySendPool.sessionDenom & (1 << 2)) && v == ((10*COIN)  +10000)) {fAccepted = true;}
            else if((legacySendPool.sessionDenom & (1 << 3)) && v == ((1*COIN)   +1000)) {fAccepted = true;}
            else if((legacySendPool.sessionDenom & (1 << 4)) && v == ((.1*COIN)  +100)) {fAccepted = true;}
            if(!fAccepted) continue;
    ...
    if(legacySendPool.GetDenominations(vOut) != legacySendPool.sessionDenom) {
    ...
    legacySendPool.SendLegacysendDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
