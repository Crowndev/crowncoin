...
void CMasternode::Check(bool forceCheck)
{
    ...
    if(!unitTest){
        ...
        CTxOut vout = CTxOut(9999.99*COIN, legacySendPool.collateralPubKey);
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorMessage, CMasternodeBroadcast &mnb, bool fOffline) {
    ...
    if(!legacySendSigner.SetKey(strKeyMasternode, strErrorMessage, keyMasternodeNew, pubKeyMasternodeNew))
    {
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    if(protocolVersion <= 99999999) {
        ...
        if(!legacySendSigner.VerifyMessage(pubkey, sig, strMessage, errorMessage)){
...
bool CMasternodeBroadcast::Sign(CKey& keyCollateralAddress)
{
    ...
    if(!legacySendSigner.SignMessage(strMessage, errorMessage, sig, keyCollateralAddress)) {
...
bool CMasternodePing::Sign(CKey& keyMasternode, CPubKey& pubKeyMasternode)
{
    ...
    if(!legacySendSigner.SignMessage(strMessage, errorMessage, vchSig, keyMasternode)) {
        LogPrintf("CMasternodePing::Sign() - Error: %s\n", errorMessage);
        return false;
    }

    if(!legacySendSigner.VerifyMessage(pubKeyMasternode, vchSig, strMessage, errorMessage)) {
        LogPrintf("CMasternodePing::Sign() - Error: %s\n", errorMessage);
        return false;
    }
