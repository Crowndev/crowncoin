...
bool CSporkManager::CheckSignature(CSporkMessage& spork)
{
    ...
    if(!legacySendSigner.VerifyMessage(pubkey, spork.vchSig, strMessage, errorMessage)){
        return false;
    }
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    if(!legacySendSigner.SetKey(strMasterPrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CMasternodePayments::Sign - ERROR: Invalid masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }

    if(!legacySendSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CMasternodePayments::Sign - Sign message failed");
        return false;
    }

    if(!legacySendSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
