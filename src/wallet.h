...
//    bool SelectCoins(int64_t nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_COINS, bool useIX = true) const;
    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nLegacysendRoundsMin, int nLegacysendRoundsMax);
    bool SelectCoinsCollateral(std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet) const ;
    bool SelectCoinsLegacy(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const;
    ...
    std::string PrepareLegacysendDenominate(int minRounds, int maxRounds);
    int GenerateLegacysendOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the Legacysend chain depth for a given input
    int GetRealInputLegacysendRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputLegacysendRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            const int rounds = pwallet->GetInputLegacysendRounds(vin);
            if(rounds >=-2 && rounds < nLegacysendRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with Legacysend. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
