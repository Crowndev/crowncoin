...
void CMasternodeMan::ProcessMasternodeConnections()
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes) {
        if(pnode->fLegacySendMaster){
            if(legacySendPool.pSubmittedToMasternode != NULL && pnode->addr == legacySendPool.pSubmittedToMasternode->addr) continue;
            LogPrintf("Closing Masternode connection %s \n", pnode->addr.ToString());
            pnode->fLegacySendMaster = false;
            pnode->Release();
        }
    }
}
...
bool CMasternodeMan::CheckMnbAndUpdateMasternodeList(CMasternodeBroadcast mnb, int& nDos) {
    ...
    if(!legacySendSigner.IsVinAssociatedWithPubkey(mnb.vin, mnb.pubkey)) {
    ...
    //  - this is checked later by .check() in many places and by ThreadCheckLegacySendPool()
