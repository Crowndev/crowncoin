...
- saction for Legacysend. These are special transactions that require a signature that only the masternodes can create.
- Legacysend now has no fees to track what-so-ever, all that will ever be in Legacysend transactions are Legacysend denominations.
...
- Split up legacysend.cpp into masternode.cpp, activemasternode.cpp and instantx.cpp
- Added modular ProcessMessages for Legacysend, Masternodes and InstantX
...
- Fixed all sorts of issues with collateral and Legacysend.
...
- Legacysend tested with 2.5 DRK successfully and I've set that as the minimum
